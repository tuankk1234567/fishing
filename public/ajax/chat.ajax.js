            ...
            success: function(response) {
                if (response.message === 'You must login') {
                    ...
                    for (var i = 0; i < response.data.chat.length; i++) {
                        $('.left').append('<div class="list" id = "' + response.data.chat[i]._id + '">\
                         <img class="imgUser" src="' + response.data.chat[i].idMaster.imageUrl + '" alt="">\
                         <span class="name">' + response.data.chat[i].idMaster.name + '</span></div>')
                    }

                    socket.emit("tao-room-chat", response.data.chat[0]._id)
                    getdatachat(response.data.chat[0]._id);
                    console.log(response.data.chat[0]._id)
