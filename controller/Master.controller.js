...
let deleteticketvip = (req, res) => {
    TicketVipModel.findOne({ _id: req.body._id }).populate('nameMaster').populate('nameFishing')
        .then(dataTicket => {
            if (dataTicket.type === "payment confirmation") {
                res.json({ message: "payment confirmation" });
            }
            var content = dataTicket.nameMaster.name + ' sorry for the inconvenience, we have some business to cancel your ticket: ' + dataTicket.name + ' ,time:' + dataTicket.date + ': ' + dataTicket.time + ' Address: ' + dataTicket.nameMaster.address;
            var mainOptions = {
                from: 'fishing2479999@gmail.com',
                to: dataTicket.nameFishing.email,
...
let paymentconfirmation = (req, res) => {
    TicketVipModel.findOne({ _id: req.body._id }).populate('nameMaster').populate('nameFishing')
        .then(dataTicket => {
            var content = dataTicket.name + ' ,time:' + dataTicket.date + ': ' + dataTicket.time + ' Address: ' + dataTicket.nameMaster.address + ' have been paid';
            var mainOptions = {
                from: 'fishing2479999@gmail.com',
                to: dataTicket.nameFishing.email,
                subject: 'Notification',
