...
let getAccount = (req, res) => {
    ...
    if (text === undefined || text === "") {
        ...
        AccountModel.find({ role: role, $or: [{ email: { $regex: text, $options: "i" } }, { name: { $regex: text, $options: "i" } }, { address: { $regex: text, $options: "i" } }] })
            .then(data1 => {
                if (data1.length <= limit) {
                    numberPage = 1;
                ...
                if (data1.length <= limit) {
                    AccountModel.find({ role: role, $or: [{ email: { $regex: text, $options: "i" } }, { name: { $regex: text, $options: "i" } }, { address: { $regex: text, $options: "i" } }] }).skip(skip).limit(limit)
                        .then(data => {
